function drift_exact_mvn_mean0(sigma0, sigmaG, sigmaH, x, y)
    ...
    drift2 = zeros(N, N);
    for i=1:N
        first = exp(0.5*x[i]^2*(-B+B^2/alpha2+D^2/beta2));
        for j=1:N
            second = exp(x[i]*y[j] * (-C+B*C/alpha2+D*E/beta2) +
            0.5*y[j]^2 * (-1/((1-rhoG^2) * sigmaG[2, 2]) + C^2/alpha2 + E^2/beta2));
            meanpartX = x[i] * (1/sqrt(sigmaG[1, 1]) -
                B/(alpha2 * sqrt(sigmaG[1, 1])) +
                D*rhoG/(beta2 * sqrt(sigmaG[2, 2])) -
                A*D/(alpha2 * beta2 * sqrt(sigmaG[1, 1]))) +
                y[j] * (-rhoG/sqrt(sigmaG[2, 2]) -
            ...
            drift1[j, i] = meanpartX * first * second;
            drift2[i, j] = meanpartY * first * second;
        end
    end
