...
function wgf_deblurring(N, Niter, lambda, I, M, sigma, v, a, b)
    ...
    for n=1:(Niter-1)
        ...
        for j=1:M
            ...
            hN[j] = mean(pdf.(Normal.(0, sigma), hSample[j, 1] .- y[n, :]) .*
                    pdf.(Beta(a, b), (x[n, :] .- hSample[j, 2] .+ v/2)./v) /v
                 );
        end
        # gradient and drift
        driftX = zeros(N, 1);
        ...
        for i=1:N
            ...
            prec = pdf.(Normal.(0, sigma), hSample[:, 1] .- y[n, i]);
            gradientX = - prec .* (a + b -2) * (a + b - 1) / v^3 .*
                        pdf.(Beta(a-1, b-1), (x[n, i] .- hSample[:, 2] .+ v/2)./v);
            gradientY = prec .* (hSample[:, 1] .- y[n, i])/(sigma^2) .*
                        pdf.(Beta(a, b), (x[n, i] .- hSample[:, 2] .+ v/2)./v) /v;
